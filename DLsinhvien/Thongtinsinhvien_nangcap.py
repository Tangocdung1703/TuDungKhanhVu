...
def calculate_average(data, student_id):
    print(f"Đang tìm kiếm sinh viên có ID: {student_id}")
    if data.size == 0:
        return "Dữ liệu không được tải."
...
def create_report(data):
    ...
    for student_id in student_ids:
        ...
        if average is not None:  # Kiểm tra nếu có giá trị điểm hợp lệ
            new_row = pd.DataFrame(
                {'ID': [student_id], 'Họ và tên': ['Trung bình cộng điểm'], 'Môn học': ['Tổng'], 'Điểm': [f"{average:.2f}"]})
...
def main():
    #global data
    file_path = 'D:\Subject\Semster 7\MaNguonMo\TuDungKhanhVu\DLsinhvien\data.csv'  # Đặt đường dẫn tuyệt đối đến file dữ liệu của bạn
    ...
    tk.Button(root, text="Tìm kiếm", command=search_action(data)).pack(pady=10) # Thêm tham số cho hàm search_action
